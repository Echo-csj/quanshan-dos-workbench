...
(function() {
  ...
  // 共享表格渲染模块（与「周度数据中心」明细同源、单一事实来源，杜绝分叉）
  // 屏幕渲染 / 导出 / 图片均沿用共享辅助函数，保证口径完全一致、零回归
  // 兼容 Node 测试：若 kpiTable 尚未加载（独立 require 视图模块），则按需载入；
  // 取 require 的返回值（已 module.exports）挂到当前 App.views，避免模块缓存/App 重赋值导致取不到（浏览器中由 index.html 脚本标签先行加载，require 未定义故为 no-op）
  App.views = App.views || {};
  if (typeof require !== 'undefined') {
    try {
      var _kt = require('../views/kpi-table.js');
      App.views.kpiTable = App.views.kpiTable || _kt || null;
    } catch (e) {}
  }
  var T = App.views.kpiTable;
  var pct = T.pct;
  var safeByDay = T.safeByDay;
  var fmtBreakdown = T.fmtBreakdown;
  // canvas 用饱和度配色（十六进制）
  ...
  function render() {
    ...
    html += T.renderTeacherKpiTable(rows, {
      selectable: isLive,
      title: '按教师',
      emptyHint: isLive ? '（请先在「课程表」导入 / 填写并保存）' : '',
      toggleExpr: function (idx) { return 'App.views.weeklyKpi.toggleTeacher(' + idx + ', this.checked)'; }
    });
    html += T.renderGroupKpiTable(groups, (groups && groups.length) ? summary : null, { onlySelected: true });

    container.innerHTML = html;
  }

  // 「按教师 / 按科组」表格统一委托共享模块 App.views.kpiTable.renderTeacherKpiTable / renderGroupKpiTable
  // （与「周度数据中心」明细同源、单一事实来源，杜绝分叉；保留 canvas 导出用的 satHex）
