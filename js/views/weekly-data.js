...
(function () {
  ...
  // 表格渲染统一委托共享模块 App.views.kpiTable（与「教师周度 KPI」同源、单一事实来源，杜绝分叉）
  // 兼容 Node 测试：若 kpiTable 尚未加载（独立 require 视图模块），则按需载入并取返回值挂到 App.views
  App.views = App.views || {};
  ...
  // 导出 XLSX 仍使用共享辅助（与屏幕表同源口径）
  var safeByDay = T.safeByDay;

  // 周度明细弹窗（只读）：按教师 + 按科组，复用共享表格渲染（与「教师周度 KPI」完全一致）
  function openDetail(weekStart) {
    var r = App.weeklyData.get(weekStart);
    if (!r) { App.util.toast('记录不存在', 'bad'); return; }
    var html = T.renderTeacherKpiTable(r.kpiByTeacher, { title: '按教师 · ' + weekStart });
    html += T.renderGroupKpiTable(r.kpiByGroup, r.campusSummary);
  ...
  function renderMonth() {
    ...
    if (!sum.weeks) {
      ...
      html += T.renderTeacherKpiTable(sum.byTeacher, { title: '按教师（跨周去重）' });
      html += T.renderGroupKpiTable(sum.groups, sum);
      html += '</div>';
    }
    html += '</div>';
    return html;
